...
#include "ms_xbit_ops.h"
#include "ms_xcharconv.h"
#include "ms_xcharconv_ryu.h"

// This implementation is dedicated to the memory of Mary and Thavatchai.

namespace msstl
{
	extern const char _Charconv_digits[];
...
to_chars_result _Integer_to_chars(
    ...
    const size_t _Buff_size = sizeof(_Unsigned) * CHAR_BIT; // enough for base 2
    ...
    case 10:
        { // Derived from _UIntegral_to_buff()
            ...
            const bool _Use_chunks = sizeof(_Unsigned) > sizeof(size_t);

            if (_Use_chunks) { // For 64-bit numbers on 32-bit platforms, work in chunks to avoid 64-bit
                                         // divisions.
                while (_Value > 0xFFFF'FFFFU) {
                    // Performance note: Ryu's division workaround would be faster here.
                    unsigned long _Chunk = static_cast<unsigned long>(_Value % 1000000000);
                    _Value               = static_cast<_Unsigned>(_Value / 1000000000);
...
inline unsigned char _Digit_from_char(const char _Ch) noexcept {
    ...
    static unsigned char _Digit_from_byte[] = {255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
    ...
    static_assert(sizeof(_Digit_from_byte) == 256, "Wrong size");
...
from_chars_result _Integer_from_chars(
    ...
    MSSTL_ASSERT(_Base >= 2 && _Base <= 36, "invalid base in from_chars()");
    ...
    const _Unsigned _Uint_max    = static_cast<_Unsigned>(-1);
    const _Unsigned _Int_max     = static_cast<_Unsigned>(_Uint_max >> 1);
    const _Unsigned _Abs_int_min = static_cast<_Unsigned>(_Int_max + 1);
...
struct _Big_integer_flt {
    ...
    static const uint32_t _Maximum_bits = 1074 // 1074 bits required to represent 2^1074
                                            + 2552 // ceil(log2(10^768))
                                            + 54; // shift space

    static const uint32_t _Element_bits = 32;

    static const uint32_t _Element_count = (_Maximum_bits + _Element_bits - 1) / _Element_bits;
...
inline uint32_t _Bit_scan_reverse(const _Big_integer_flt& _Xval) noexcept {
    ...
    ::msstl::BitScanReverse(_Index, _Xval._Mydata[_Bx]); // assumes _Xval._Mydata[_Bx] != 0
...
inline bool _Multiply_by_power_of_ten(_Big_integer_flt& _Xval, const uint32_t _Power) noexcept {
    ...
    static uint32_t _Large_power_data[] = {0x540be400, 0x00000002, 0x63100000, 0x6bc75e2d, 0x00000005,
    ...
    static const _Unpack_index _Large_power_indices[] = {{0, 0, 2}, {2, 0, 3}, {5, 0, 4}, {9, 1, 4}, {13, 1, 5},
    ...
    for (uint32_t _Large_power = _Power / 10; _Large_power != 0;) {
        const uint32_t _Current_power =
            (std::min) (_Large_power, static_cast<uint32_t>(sizeof(_Large_power_indices) / sizeof(_Large_power_indices[0])));
    ...
    static uint32_t _Small_powers_of_ten[9] = {
...
inline uint64_t _Divide(_Big_integer_flt& _Numerator, const _Big_integer_flt& _Denominator) noexcept {
    ...
    MSSTL_ASSERT_SINGLE(_Denominator._Myused != 0); // Division by zero
...
inline uint64_t _Right_shift_with_rounding(
    ...
    const uint32_t _Total_number_of_bits = 64;
    if (_Shift >= _Total_number_of_bits) {
        if (_Shift == _Total_number_of_bits) {
            const uint64_t _Extra_bits_mask = (1ULL << (_Total_number_of_bits - 1)) - 1;
            const uint64_t _Round_bit_mask  = (1ULL << (_Total_number_of_bits - 1));
...
inline void _Accumulate_decimal_digits_into_big_integer_flt(
    ...
    for (const uint8_t* _It = _First_digit; _It != _Last_digit; ++_It) {
        if (_Accumulator_count == 9) {
            const bool _Success1 = _Multiply(_Result, 1000000000); // assumes no overflow
			MSSTL_ASSERT_SINGLE(_Success1);
			(void)_Success1;
            const bool _Success2 = _Add(_Result, _Accumulator); // assumes no overflow
			MSSTL_ASSERT_SINGLE(_Success2);
			(void)_Success2;
    ...
    if (_Accumulator_count != 0) {
        const bool _Success3 =
            _Multiply_by_power_of_ten(_Result, _Accumulator_count); // assumes no overflow
		MSSTL_ASSERT_SINGLE(_Success3);
		(void)_Success3;
        const bool _Success4 = _Add(_Result, _Accumulator); // assumes no overflow
		MSSTL_ASSERT_SINGLE(_Success4);
		(void)_Success4;
    }
}
...
errc _Convert_decimal_string_to_floating_type(
    ...
    if (_Fractional_shift > 0) {
		...
		MSSTL_ASSERT_SINGLE(_Shift_success1);
...
from_chars_result _Ordinary_floating_from_chars(const char* const _First, const char* const _Last,
    ...
    const ptrdiff_t _Maximum_adjustment = 1000000;
    const ptrdiff_t _Minimum_adjustment = -1000000;
    ...
    const int _Maximum_temporary_decimal_exponent = 5200;
	const int _Minimum_temporary_decimal_exponent = -5200;
    ...
    if (_Mantissa_it == _Mantissa_first) {
		MSSTL_ASSERT_SINGLE(_Has_zero_tail);
...
from_chars_result _Floating_from_chars(
    ...
    MSSTL_ASSERT(_Fmt == chars_format::general || _Fmt == chars_format::scientific || _Fmt == chars_format::fixed
...
to_chars_result _Floating_to_chars_hex_precision(
	...
	const int _Full_precision         = std::is_same_v<_Floating, float> ? 6 : 13;
    const int _Adjusted_explicit_bits = _Full_precision * 4;
    ...
    if (std::is_same_v<_Floating, float>) {
    ...
    // * Print the leading hexit, then mask it away.
    {
		...
		MSSTL_ASSERT_SINGLE(_Nibble < 3);
        const char _Leading_hexit = static_cast<char>('0' + _Nibble);

        *_First++ = _Leading_hexit;

		const _Uint_type _Mask = (_Uint_type{1} << _Adjusted_explicit_bits) - 1;
    ...
    if (_Precision > 0) {
        *_First++ = '.';
        ...
        for (;;) {
			MSSTL_ASSERT_SINGLE(_Number_of_bits_remaining >= 4);
			MSSTL_ASSERT_SINGLE(_Number_of_bits_remaining % 4 == 0);
            _Number_of_bits_remaining -= 4;

            const uint32_t _Nibble = static_cast<uint32_t>(_Adjusted_mantissa >> _Number_of_bits_remaining);
			MSSTL_ASSERT_SINGLE(_Nibble < 16);
    ...
    *_First++ = _Sign_character;
    ...
    return msstl::to_chars(_First, _Last, _Absolute_exponent);
}
...
to_chars_result _Floating_to_chars_hex_shortest(
    ...
    *_First++ = 'p';
    ...
    return msstl::to_chars(_First, _Last, static_cast<uint32_t>(_Unbiased_exponent));
}
...
struct _General_precision_tables<float> {
    static const int _Max_special_P = 7;

	static const uint32_t _Special_X_table[63];;

    static const int _Max_P = 39;
...
struct _General_precision_tables<double> {
    static const int _Max_special_P = 15;

	static const uint64_t _Special_X_table[195];

    static const int _Max_P = 309;
...
inline to_chars_result _Floating_to_chars_general_precision(
    ...
    const _Uint_type* const _Table_lower_bound = [=] {
        if (!std::is_same_v<_Floating, float>) {
            if (_Precision > 155) { // threshold determined via profiling
                return std::lower_bound(_Table_begin, _Table_end, _Uint_value, std::less<_Uint_type>());
            }
        }
    ...
    if (_Use_fixed_notation) {
		...
		MSSTL_ASSERT_SINGLE(_Buf_result.ec == errc{});
...
to_chars_result _Floating_to_chars(
    ...
    if (_Overload == _Floating_to_chars_overload::_Plain) {
		MSSTL_ASSERT_SINGLE(_Fmt == chars_format{}); // plain overload must pass chars_format{} internally
    } else {
        MSSTL_ASSERT(_Fmt == chars_format::general || _Fmt == chars_format::scientific || _Fmt == chars_format::fixed
                        || _Fmt == chars_format::hex,
    ...
    } else if (_Overload == _Floating_to_chars_overload::_Format_only) {
    ...
    } else if (_Overload == _Floating_to_chars_overload::_Format_precision) {
        switch (_Fmt) {
